...
module Rouge
  module Lexers
    class Fish < RegexLexer
      ...
      KEYWORDS = %w(
        if else while for then return function end and or not
        switch case
      ).join('|')

      BUILTINS = %w(
        delete-or-exit dirh down-or-search fish_command_not_found_setup
        fish_config fish_default_key_bindings fish_prompt fish_right_prompt
        fish_sigtrap_handler fish_update_completions funced funcsave isatty
        nextd nextd-or-forward-word open prevd prevd-or-backward-word
        prompt_pwd psub setenv up-or-search vared 
      ...
      state :basic do
        ...
        # TODO this is set and setenv jobs
        rule %r/(\b\w+)(=)/ do
